...
export function patchAxios(cfg: ResolvedConfig, axiosInstance?: any): () => void {
    ...
    if (!instance) {
        try {
            // works in CJS environments
            const mod = require("axios");
            instance = mod.default ?? mod;
        } catch {
            return noop; // ESM — caller must pass instance
        }
    }
    ...
    const resInterceptor = instance.interceptors.response.use(
        (response: any) => { response.config?.__nodeye_done__?.(); return response; },
        (error: any) => { error.config?.__nodeye_done__?.(); return Promise.reject(error); }
    );
